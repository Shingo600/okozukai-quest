...
## `harness-check.mjs` の検出ルール一覧

`npm run harness:check` で実行される静的検査の中身。
...
- `.env.local.example`
- `package.json`（`scripts.build` と `scripts.typecheck` の存在も確認）

### 危険パターンスキャン（`.ts/.tsx/.js/.jsx/.mjs/.cjs`）

| ID | レベル | 内容 |
|---|---|---|
| `eval` | error | `eval(` 使用禁止 |
| `new-function` | error | `new Function(` 使用禁止 |
| `console-log-process-env` | error | `console.log(process.env...)` で環境変数漏洩 |
| `console-log-secret` | error | `console.log` に `token / secret / password / apiKey / API_KEY` |
| `jwt-literal` | error | `"eyJ..."` 形式の JWT 風リテラル（Supabase anon key 等の誤コミット）|
| `setstate-closure-in-timer` | warn | `setTimeout/setInterval` の中で `state` を直接参照（v0.2.1 で同種バグ修正）|

### 数量チェック

- `console.log` の出現が **20 箇所を超えるとエラー**（開発残骸の混入防止）

### 型チェック

`harness:check` は最後に `npm run typecheck` を呼ぶ。`typecheck` は以下のフラグで動作:
...
- 同じ種類のバグが 2 回以上起きたら、検出ルールに昇格させる
- 警告（warn）から始めて、十分削減されたらエラー（error）に昇格
- false positive が出やすいパターンは warn 止まりにし、必要なら個別に `// eslint-disable` 風のコメントで suppress（現状未対応、将来導入を検討）
