...
export function UserSwitch() {
  const { state, setCurrentUser, resetDemo } = useStore();
  return (
      ...
      <div className="w-full max-w-sm space-y-3">
        <Section title="こども" color="from-kid-green to-emerald-300">
          {state.users.filter((u) => u.role === "child").map((u) => (
            <UserCard key={u.id} avatar={u.avatar} name={u.name} sub={`Lv.${u.level} ・ ${u.allowanceBalance}円`} onClick={() => setCurrentUser(u.id)} />
          ))}
        </Section>
        <Section title="おとな" color="from-parent-purple to-parent-purpleDeep">
          {state.users.filter((u) => u.role !== "child").map((u) => (
            <UserCard key={u.id} avatar={u.avatar} name={u.name} sub={u.role === "mother" ? "ママ" : "パパ"} onClick={() => setCurrentUser(u.id)} />
          ))}
        </Section>
      </div>
      <button onClick={resetDemo} className="mt-8 text-xs text-gray-400 underline">デモデータを初期化</button>
    </div>
  );
}
